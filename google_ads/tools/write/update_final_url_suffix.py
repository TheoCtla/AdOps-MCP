"""Tool: google_ads_update_final_url_suffix.

Modifie le suffixe d'URL finale (UTM) au niveau account, campaign ou
ad group.
"""
...
TOOL_NAME = "google_ads_update_final_url_suffix"
...
TOOL_DEFINITION = Tool(
    name=TOOL_NAME,
    description=(
        "Update the final URL suffix on a Google Ads account, campaign, or ad group. The "
        "suffix is appended to all final URLs at that level.\n"
        "\n"
        "Returns a JSON confirmation with the level, resource, and the new suffix.\n"
        "\n"
        "Use this tool to set UTM parameters or tracking suffixes that are appended to "
        "landing page URLs (e.g. 'utm_source=google&utm_medium=cpc&utm_campaign="
        "{campaignid}'). Set at ACCOUNT level for global suffixes, or at CAMPAIGN/AD_GROUP "
        "for specific overrides. Pass an empty string to clear the suffix.\n"
        "\n"
        "⚠️ This tool MODIFIES data. The suffix change takes effect immediately and impacts "
        "all ad clicks on the affected level."
    ),
    inputSchema={
        "type": "object",
        "properties": {
            ...
            "level": {
                ...
                "description": "Level to set the final URL suffix on.",
            },
            "resource_id": {
                "type": "string",
                "description": (
            ...
            "final_url_suffix": {
                "type": "string",
                "description": (
                    "The suffix to append to final URLs "
                    "(e.g. 'utm_source=google&utm_medium=cpc'). "
                    "Pass empty string to clear."
                ),
            },
        },
        "required": ["customer_id", "level", "final_url_suffix"],
...
async def handler(arguments: dict[str, Any]) -> list[TextContent]:
    """Handler for google_ads_update_final_url_suffix."""
    args = arguments or {}
    ...
    final_url_suffix = args.get("final_url_suffix")
    if final_url_suffix is None:
        return error_payload("Paramètre 'final_url_suffix' requis.")
    ...
    try:
        ...
        log.exception("Erreur inattendue dans google_ads_update_final_url_suffix")
    ...
    payload = {
        "success": True,
        "action": "UPDATED_FINAL_URL_SUFFIX",
        "level": level,
