"""Tool: google_ads_pause_ad_group.

Met en pause un ad group. Stoppe la diffusion de toutes ses annonces
sans toucher aux autres ad groups de la campagne.
"""
...
TOOL_NAME = "google_ads_pause_ad_group"


TOOL_DEFINITION = Tool(
    name=TOOL_NAME,
    description=(
        "Pause a Google Ads ad group, immediately stopping ad delivery for all ads in that "
        "group without affecting other ad groups in the campaign.\n"
        "\n"
        "Returns a JSON confirmation with success status, action taken, and the resource name.\n"
        "\n"
        "Use this tool when the user wants to stop a specific ad group from spending while "
        "keeping the rest of the campaign active.\n"
        "\n"
        "⚠️ This tool MODIFIES data. Pausing an ad group stops all its ad delivery "
    ...
    inputSchema={
        ...
        "properties": {
            ...
            "ad_group_id": {
                "type": "string",
                "description": (
                    "Numeric ad group ID to pause. Use "
...
async def handler(arguments: dict[str, Any]) -> list[TextContent]:
    """Handler for google_ads_pause_ad_group."""
    args = arguments or {}
    ...
    try:
        ...
        log.exception("Erreur inattendue dans google_ads_pause_ad_group")
