"""Tool: google_ads_get_hour_of_day_performance.

Performance par heure de la journée (0-23). Fournit un breakdown
(heure, campagne) et optionnellement un agrégat par heure toutes
...
from google_ads.queries import HOUR_OF_DAY_PERFORMANCE_QUERY


log = logging.getLogger(__name__)

TOOL_NAME = "google_ads_get_hour_of_day_performance"


TOOL_DEFINITION = Tool(
    name=TOOL_NAME,
    description=(
        "Fetch performance metrics segmented by hour of day (0-23) for a Google Ads "
        "advertiser account.\n"
        "\n"
        "Returns a JSON object with `customer_id`, `date_range`, `filters`, `totals`, "
        "`breakdown` (one entry per (hour, campaign) pair, sorted by hour asc), and — when no "
        "campaign_id is provided — `aggregated_by_hour` (24 entries, one per hour 0-23, "
        "summed across all campaigns). Each entry contains: hour (int 0-23), campaign_id, "
        "campaign_name (breakdown only), impressions, clicks, cost (euros), conversions, "
        "conversion_value, ctr, avg_cpc, cpa. Hours with no data still appear in "
        "`aggregated_by_hour` with zeroed counters and null ratios.\n"
        "\n"
        "Use this tool to find the best and worst hours for conversions, identify off-hours "
        "waste, or set ad scheduling bid adjustments. Only ENABLED campaigns are included. "
        ...
        "properties": {
            ...
            "campaign_id": {
                ...
                "description": (
                    ...
                    "contains only `breakdown` (no aggregated_by_hour)."
                ),
            },
            "date_from": {
                "type": "string",
...
async def handler(arguments: dict[str, Any]) -> list[TextContent]:
    """Handler for google_ads_get_hour_of_day_performance."""
    args = arguments or {}
    ...
    query = HOUR_OF_DAY_PERFORMANCE_QUERY.format(
        date_from=date_from,
        date_to=date_to,
    ...
    hour_agg: dict[int, dict[str, float]] = defaultdict(
        lambda: {
            "impressions": 0,
            "clicks": 0,
            "cost_micros": 0,
    ...
    try:
        ...
        log.exception("Erreur inattendue dans google_ads_get_hour_of_day_performance")
    ...
    if not campaign_id:
        ...
        payload["aggregated_by_hour"] = aggregated
