"""Tool: meta_ads_update_adset_budget.

Modifie le budget d'un ad set Meta Ads (daily ou lifetime).
"""
...
TOOL_NAME = "meta_ads_update_adset_budget"


TOOL_DEFINITION = Tool(
    name=TOOL_NAME,
    description=(
        "Update the budget of a Meta Ads ad set (daily or lifetime). Provide exactly one "
        "of daily_budget or lifetime_budget.\n"
        "\n"
        "Returns a JSON confirmation with the new budget value in euros.\n"
        "\n"
        "Use this tool when the user asks to change an ad set's budget. The value is in "
        "euros and is automatically converted to centimes.\n"
        "\n"
...
async def handler(arguments: dict[str, Any]) -> list[TextContent]:
    """Handler for meta_ads_update_adset_budget."""
    args = arguments or {}
    ...
    try:
        ...
        if daily_raw is not None:
            adset[AdSet.Field.daily_budget] = euros_to_cents(budget_eur)
        else:
            adset[AdSet.Field.lifetime_budget] = euros_to_cents(budget_eur)
        adset.remote_update()
    except Exception as ex:
        log.exception("Erreur dans meta_ads_update_adset_budget")
